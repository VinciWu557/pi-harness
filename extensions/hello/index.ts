...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ctx.ui.notify("pi-harness 已加载", "info");
  });
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    for (const { pattern, msg } of dangerous) {
      if (pattern.test(cmd)) {
        const ok = await ctx.ui.confirm("⚠️ 安全检查", `${msg}\n\n命令: ${cmd}\n\n是否允许执行？`);
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      language: Type.Optional(
        Type.String({ description: "语言: zh, en, ja (默认 zh)" })
      ),
    }),
  ...
  pi.registerCommand("harness-status", {
    description: "查看 pi-harness 状态",
    handler: async (_args, ctx) => {
      ctx.ui.notify("pi-harness v0.1.0 运行中", "info");
      ctx.ui.setStatus("harness", "✅ pi-harness 已激活");
    },
  });
}
