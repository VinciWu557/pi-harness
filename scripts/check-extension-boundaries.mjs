...
const WORKSPACE_PACKAGE_RE = /^@vinciwu557\//;
...
const workspaceDirectories = ["extensions", "skills"];
const activePackages = findActivePackages(workspaceDirectories);
const failures = [];

for (const pkg of activePackages) {
	checkPackageDependencies(pkg);
	checkSourceImports(pkg);
}
...
console.log(
	`Package boundary check passed: ${activePackages.length} active packages have no cross-package dependencies.`,
);

function findActivePackages(directories) {
	const packages = [];
	...
	for (const dir of directories) {
		...
		for (const entry of fs.readdirSync(fullDir, { withFileTypes: true })) {
			...
			const packagePath = path.join(fullDir, entry.name, "package.json");
...
function checkPackageDependencies(pkg) {
	for (const field of DEPENDENCY_FIELDS) {
		...
		for (const dependencyName of Object.keys(dependencies)) {
			if (!isForbiddenPackageReference(pkg.name, dependencyName)) continue;

			failures.push(
				`${relative(pkg.packagePath)} ${field} must not reference ${dependencyName}.`,
			);
		}
	}
}

function checkSourceImports(pkg) {
	const sourceDirectory = path.join(pkg.directory, "src");
	...
	for (const sourcePath of listSourceFiles(sourceDirectory)) {
		...
		for (const specifier of moduleSpecifiers(sourcePath, source)) {
			if (!isForbiddenPackageReference(pkg.name, specifier)) continue;
...
function isForbiddenPackageReference(packageName, specifier) {
	if (specifier === packageName || specifier.startsWith(`${packageName}/`)) return false;
	return WORKSPACE_PACKAGE_RE.test(specifier);
}
